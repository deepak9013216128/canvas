...
class Game {
  ...
  init() {
    ...
    this.gridSize = {
      rows: Math.floor(this.canvas.height / sourceSize.h),
      cols: Math.floor(this.canvas.width / sourceSize.w),
      width: sourceSize.w,
      height: sourceSize.h,
    };
    const topleft = { x: 32, y: 64 };
    ...
    try {
      if (sprite.index == index && !sprite.checked) {
        ...
        if (boundaryCheck(row - 1, col))
          connected.concat(
            this.getConnectedSprites(index, row - 1, col, connected)
          );
        if (boundaryCheck(row + 1, col))
          connected.concat(
            this.getConnectedSprites(index, row + 1, col, connected)
        ...
        if (boundaryCheck(row, col - 1))
          connected.concat(
            this.getConnectedSprites(index, row, col - 1, connected)
          );
        if (boundaryCheck(row, col + 1))
          connected.concat(
            this.getConnectedSprites(index, row, col + 1, connected)
...
class Sprite {
  ...
  render() {
    ...
    this.context.drawImage(
      ...
      this.x -
        (this.width - this.frameData.spriteSourceSize.x) *
          this.scale *
          this.anchor.x,
      this.y -
        (this.height - this.frameData.spriteSourceSize.y) *
          this.scale *
          this.anchor.y,
