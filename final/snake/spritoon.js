...
class Actor {
  ...
  update(dt) {
    if (this.action != null) {
      ...
      if (
        this.action.loop != null &&
        this.action.loop &&
        this.action.duration != null &&
        this.currentTime > this.action.duration
      ) {
        this.currentTime = 0;
      }
  ...
  render(context) {
    ...
    for (let i = 0; i < this.layers.length; i++)
      this.layers[i].render(context, this.flipped);
  }
}
...
class Anim {
          ...
          default:
            if (frames[i].startsWith("h")) {
              ...
              if (!isNaN(secs)) {
                ...
                console.error(
                  `Anim.parseframes ${frames[i]} does not contain a number.`
                );
              }
            } else {
              console.error(
                "Anim.parseFrames found unexpected string " + frames[i]
              );
            }
            break;
        }
  ...
  update(dt = 0) {
    ...
    if (this.frame == null) {
      console.error(
        `Anim null frame for index ${index}, frames count:${this.frames.length}`
      );
    }
  }
}
...
class AnimSprite {
  ...
  set anim(value) {
    ...
    if (
      this._anim != null &&
      this._anim.motion != null &&
      this._anim.motion.easing != null
    ) {
      let options = value.motion.easing;
      //(start, end, duration, startTime=0, type='linear')
      this._anim.easing = new Easing(
        value.y,
        value.y + options.change,
        options.duration,
        0,
        options.type
      );
  ...
  update(dt) {
    if (this._anim != null) {
      ...
      if (this.physicsBody == null && this._anim.motion != null) {
        this.x +=
          (this.flipped ? -this._anim.motion.x : this._anim.motion.x) * dt;
  ...
  get boundingBox() {
    ...
    pt.x -=
      (this.frameData.sourceSize.w - (this.frameData.sourceSize.w - w) / 2) *
      this.scale *
      this.anchor.x;
    pt.y -=
      (this.frameData.sourceSize.h - (this.frameData.sourceSize.h - h) / 2) *
      this.scale *
      this.anchor.y;
  ...
  render() {
    ...
    let canvasOffset =
      this.canvasOffset == null ? { x: 0, y: 0 } : this.canvasOffset;
    ...
    if (this.physicsBody != null) {
      ...
      this.context.drawImage(
        ...
        frame.w,
        frame.h,
        -(frame.w / 2) * this.scale,
        -(frame.h / 2) * this.scale,
        frame.w * this.scale,
        frame.h * this.scale
      );
    ...
    if (this.debug) {
      ...
      const centre = new Vertex(
        this.x + canvasOffset.x,
        this.y + canvasOffset.y
      );
      const size = 5;
...
class Sprite {
    ...
    if (options.node != null) {
      ...
      if (options.config != null) {
        ...
        if (config.anchor != null) {
          this.anchor = {
            x: Number(config.anchor.x),
            y: Number(config.anchor.y),
          };
        }
  ...
  set index(value) {
    ...
    if (this.game == undefined && !this.frames.isArray()) {
      console.error(
        "Sprite trying to set index when there is no access to the frames array"
      );
    }
    ...
    if (formatPos.start == -1) {
      console.error(
        "Sprite trying to set index when _frame does not contain formatting information"
      );
    ...
    if (formatPos.end == -1) {
      console.error(
        "Sprite trying to set index when _frame does not contain correnct formatting information"
      );
      return; //Not correct formatting
    }
    let formatStr = this.frameName.substring(
      formatPos.start + 1,
      formatPos.end
    );
  ...
  get offset() {
    ...
    try {
      result = new Vertex(
        this.frame.w * scale * this.anchor.x,
        this.frame.h * scale * this.anchor.y
      );
    } catch (e) {
...
class TextBlock {
  constructor(context, text, rect, options = {}) {
    ...
    this.alignment =
      options.alignment == undefined ? "center" : options.alignment;
...
class Action {
  ...
  addKey(layerName, channelName, time, value) {
    ...
    if (window.spritoon.app.timeline != null)
      window.spritoon.app.timeline.data = layer;
  }
        ...
        for (let i = 0; i < channel.length; i++) {
          ...
          if (time == key.time) {
            if (channelName == "x" || channelName == "y") {
              sprite[channelName] =
                sprite.rest.position[channelName] + key.value;
            } else {
