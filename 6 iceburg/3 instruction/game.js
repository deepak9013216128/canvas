...
class Game {
  ...
  init() {
    ...
    this.config.jump = {
      x: this.config.iceberg.col * (fps / 11),
      y: this.config.iceberg.row * (fps / 11),
    };
    ...
    for (let row of this.icebergs) {
      for (let iceberg of row) {
        for (let anim of iceberg.anims) {
          anim.motion.x =
            anim.motion.x > 0 ? this.config.speed : -this.config.speed;
        }
      }
    }
    this.startTime = Date.now();
    ...
    for (let row of this.icebergs) {
      for (let iceberg of row) {
        if (iceberg.hitTest(pos)) {
          ...
          if (this.bear.x > left && this.bear.x < right) {
            ...
            if (!anim.paused && anim.frames.length - anim.index > 20) {
              this.bear.iceberg = {
                sprite: iceberg,
                offset: new Vertex(
                  this.bear.x - iceberg.x,
                  this.bear.y - iceberg.y
                ),
              };
              let i = 0;
          ...
          console.log(
            `Found iceberg x:${left}<${this.bear.x}<${right} paused:${anim.paused} frame:${anim.index} of ${anim.frames.length} iceberg:${this.bear.iceberg}`
          );
          found = true;
          break;
        }
  ...
  loseLife(fall = false) {
    ...
    if (fall) {
      ...
      const row = Math.floor(
        (this.bear.y - this.config.iceberg.y) / this.config.iceberg.row
      );
  ...
  render() {
    ...
    if (this.state == "initialised") {
      ...
      const blockText = new TextBlock(
        this.context,
        "Use the arrow buttons to control the polar bear. The aim is to cross the iceflow by jumping from iceberg to iceberg.",
        bb,
        this.txtoptions
      );
      ...
      const blockText = new TextBlock(
        this.context,
        "Fall in the water or disappear off the sides and you lose a life. Collect fish to gain a life\n Click to start.",
        bb,
        this.txtoptions
      );
    } else if (this.state == "gameover") {
