...
class Game {
  constructor() {
    ...
    this.states = {
      ...
      turn: {
        ...
        loop: false,
        motion: { x: 0, y: 0 },
        oncomplete: { state: "walk", flip: true, move: { x: 40, y: 0 } },
      ...
      fall: {
        ...
        loop: false,
        motion: { x: 0, y: 0 },
        oncomplete: { state: "static", flip: true },
        userinteraction: false,
        fps: 12,
      },
      pour: {
        ...
        loop: false,
        motion: { x: 0, y: 0 },
        oncomplete: { state: "static", flip: false },
        userinteraction: false,
        fps: 12,
      },
    };
  ...
  tap(evt) {
    ...
    if (
      this.bucket.state.userinteraction != null &&
      !this.bucket.state.userinteraction
    )
      return;
    ...
    else if (dx > 0.75) this.bucket.setState("walk", false);
    else if (dy < 0.25) this.bucket.setState("pour", false);
    else if (dy > 0.75) this.bucket.setState("fall", true);
  }
