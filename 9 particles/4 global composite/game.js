...
class Emitter {
  constructor(options) {
    this.emissionRate =
      options.emissionRate != undefined ? options.emissionRate : 33;
    this.particleCount =
      options.particleCount != undefined ? options.particleCount : 100;
    this.position =
      options.x != undefined ? { x: options.x, y: options.y } : { x: 0, y: 0 };
    this.life = options.life != undefined ? options.life : 3;
    this.speed = options.speed != undefined ? options.speed : 10;
    this.speedVariation =
      options.speedVariation != undefined ? options.speedVariation : 0.6;
    this.angleVariation =
      options.angleVariation != undefined && options.angle != undefined
    ...
    this.particleLife =
      options.particlelife != undefined ? options.particlelife : 1;
    this.scaleWithAge =
      options.scaleWithAge != undefined ? options.scaleWithAge : true;
  ...
  init(options) {
    ...
    const radialAccel =
      options.radialAccel != undefined ? options.radialAccel : 0;
  ...
  update(dt) {
    ...
    if (this.spawnTime > this.spawnInterval) {
      for (let particle of this.particles) {
        if (particle.life <= 0) {
          const speed =
            this.speed * (1 - this.speedVariation) +
            this.speedVariation * Math.random() * this.speed;
...
class Particle {
  ...
  update(dt) {
    ...
    this.opacity =
      ratio > fadeIn
        ? (this.origin.opacity * (1.0 - ratio)) / (1.0 - fadeIn)
        : this.origin.opacity * (ratio / fadeIn);
