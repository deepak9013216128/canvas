...
class Game {
  ...
  checkTile(tile, black) {
    return (
      (black && tile.frameData.filename == "tile_001.png") ||
      (!black && tile.frameData.filename == "tile_010.png")
    );
  }
  ...
  render() {
    ...
    for (
      let x = padding - cellsize, col = -1;
      x < this.canvas.width;
      x += cellsize, col++
    ) {
      for (
        let y = padding - cellsize, row = -1;
        y < this.canvas.height;
        y += cellsize, row++
      ) {
        this.context.globalAlpha =
          (row >= 3 && row <= 4 && col >= 3 && col <= 4) ||
          (row == this.computer.row && col == this.computer.col)
            ? 0.7
            : 1.0;
    ...
    if (this.state == "gameover") {
      ...
      const rect = new Rect(
        this.canvas.width / 2 - this.canvas.width / 6,
        this.canvas.height / 2 - this.canvas.height / 8,
        this.canvas.width / 3,
