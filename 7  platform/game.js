class Game {
  ...
  init() {
    ...
    this.anims.push(
      new Anim("jump", {
        ...
        motion: {
          x: 100,
          easing: { type: "outQuad", change: -150, duration: 18 / 30 },
        },
        loop: false,
        oncomplete: function () {
    ...
    this.anims.push(
      new Anim("drop", {
        ...
        motion: {
          x: 150,
          easing: { type: "inQuad", change: 150, duration: 18 / 30 },
        },
        loop: false,
        oncomplete: function () {
          game.checkFloor();
    ...
    this.anims.push(
      new Anim("kick", {
        ...
        motion: {
          x: 50,
          easing: { type: "projectile", change: -30, duration: 25 / 30 },
        },
        loop: false,
        oncomplete: function () {
          game.checkFloor();
  ...
  mousedown(evt) {
    ...
    let mouseData = {
      ...
      swipePosition: new Vertex(mousePos.x, mousePos.y),
      clickTime: Date.now(),
    };
  ...
  mousemove(evt) {
    ...
    if (this.mouseData != null) {
      if (
        this.mouseData.down &&
        this.xbloke.onground &&
        this.mouseData.swipePosition.y - mousePos.y > 100
      ) {
  ...
  checkFloor() {
    let rect1 = new Rect(
      this.xbloke.x,
      this.xbloke.y,
      this.xbloke.frameData.frame.w,
      this.xbloke.frameData.frame.h
    );
    let delta = 10;
  ...
  refresh() {
    ...
    if (this.mouseData != null && this.mouseData.down) {
      if (
        now - this.mouseData.clickTime > 1000 &&
        this.xbloke.animName == "walk"
      ) {
  ...
  drawPlatforms() {
    ...
    for (let i = 0; i < this.bitsData.platforms.length; i++) {
      ...
      this.context.drawImage(
        ...
        frame.w,
        frame.h,
        platform.x + this.position.x,
        platform.y + this.position.y,
        frame.w,
        frame.h
      );
    }
    ...
    for (let i = 0; i < this.bitsData.diamonds.length; i++) {
      ...
      if (diamond.found == null) {
        this.context.drawImage(
          ...
          frame.w,
          frame.h,
          diamond.x + this.position.x,
          diamond.y + this.position.y,
          frame.w * scale,
          frame.h * scale
        );
        ...
        if (opacity > 0) {
          ...
          this.context.drawImage(
            ...
            frame.w,
            frame.h,
            diamond.x + this.position.x - delta / 2,
            diamond.y + this.position.y - delta / 2,
            frame.w * dscale,
            frame.h * dscale
          );
  ...
  constrainBackground() {
    ...
    let position = new Vertex(
      centre.x - this.xbloke.x,
      centre.y - this.xbloke.y
    );
    let blend = 0.3;
    let easePosition = new Vertex(
      this.position.x * (1 - blend) + position.x * blend,
      this.position.y * (1 - blend) + position.y * blend
    );
