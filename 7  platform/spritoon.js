...
class AnimSprite {
  ...
  get offset() {
    ...
    return {
      x: (w - (w - this.frameData.frame.w) / 2) * scale * this.anchor.x,
      y: (h - (h - this.frameData.frame.h) / 2) * scale * this.anchor.y,
    };
  }
...
class Sprite {
  ...
  setRest(restore = true) {
    ...
    if (restore) {
      if (this.rest != null) {
        this.frameData =
          window.spritoon.app.spriteData.frames[this.rest.imageIndex];
